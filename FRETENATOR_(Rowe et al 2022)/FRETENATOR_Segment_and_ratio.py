...
def previewDialog(imp):
	...
	gd.addCheckbox("Create pretty nearest point projection with outlines (SLOW)? ", False)
	gd.showDialog()
	...
	while ((not gd.wasCanceled()) and not (gd.wasOKed())):
		...
		Thread.sleep(200)
	labelPrevImp.close()
	makeNearProj = gd.checkboxes.get(2).getState()
	return segmentChannel, donorChannel, acceptorChannel, acceptorChannel2, thresholdMethod, maxIntensity, gaussianSigma, largeDoGSigma, topHat, topHatSigma, manualSegment, manualThreshold, makeNearProj
...
print(options)
segmentChannel, donorChannel, acceptorChannel, acceptorChannel2, thresholdMethod, maxIntensity, gaussianSigma, largeDoGSigma, topHat, topHatSigma, manualSegment, manualThreshold, makeNearProj=options
...
if makeNearProj == True:
	...
	IJ.setMinAndMax(nearZImpOutlines, 500, 3000)
